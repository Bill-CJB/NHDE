    ...
    def _train_one_batch(self, batch_size):
        ...
        for st in range(self.trainer_params['solving_times']):
            ...
            if sols.shape[1] > self.trainer_params['neighbor']:
                ...
                ind = torch.topk(ws, self.trainer_params['neighbor'], dim=-1, largest=True).indices
            ...
            while not done:
                ...
                action_w_finisehd[state.finished] = self.env_params[
                    ...
                    hv_reward[b_i, i_i] = hypervolume(
                        -torch.cat((s_[b_i, :, :], r_[b_i, i_i, None]), dim=0).cpu().numpy().astype(
                            float)).compute(-self.env_params['ref'].numpy()) / (
                                (self.env_params['ideal'][0] - self.env_params['ref'][0]) * (self.env_params['ideal'][1] - self.env_params['ref'][1]))
