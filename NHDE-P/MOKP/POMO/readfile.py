...
def write_hv(f, data_dir, problem_size, ref, result_folder):
    ...
    print(f"MO{PROBLEM}{problem_size} Aug:{aug_factor} Finetune")
    ...
    aug_factor = 8
    # sols_file = f"./result/{TEST_DIR}/{data_dir}/PMOCO(aug)_all_mean_sols.txt"
    sols_file = f"./result/{TEST_DIR}/{data_dir}/PMOCO(aug)_all_mean_sols_n{problem_size}.txt"

    hv_file, result_file = get_filename(problem_size, sols_file)
    hvs = cal_hvs(sols_file, ref)

    print(f"MO{PROBLEM}{problem_size} Aug:{aug_factor} ")
    print('HV Ratio: {:.4f}'.format(hvs.mean()))
    np.savetxt(F"{result_folder}/{hv_file}", hvs,
               delimiter='\t', fmt="%.4f")
    f.write(f"Aug Factor: {aug_factor}\n")
...
def main(problem_size, data_dir, aug_data_dir):
    ...
    print(f"MOKP{problem_size} Aug:{aug_factor} ", 'HV Ratio: {:.4f}'.format(hvs.mean()),
...
if __name__ == "__main__":
    data_dir = "20220718_225946_test_kp_n50"
    aug_data_dir = ""
    main(50, data_dir, aug_data_dir)
    data_dir = "20220718_230049_test_kp_n100"
    aug_data_dir = ""
    main(100, data_dir, aug_data_dir)
    data_dir = "20220720_113230_test_kp_n200"
    aug_data_dir = ""
