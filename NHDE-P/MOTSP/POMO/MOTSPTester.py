    ...
    def _test_one_batch(self, shared_probelm, pref, batch_size, episode):
        ...
        for st in range(pref.shape[0]):
            ...
            with torch.no_grad():
                ...
                self.model.pre_forward(reset_state, sel_aug, sel_mask_aug)  # aug
                # self.model.pre_forward(reset_state, sel_aug / 85 * 65, sel_mask_aug)  # test MOTSP150
                # self.model.pre_forward(reset_state, sel_aug / 115 * 65, sel_mask_aug)  # test MOTSP200

            state, reward, done = self.env.pre_step()

            while not done:
                selected, _ = self.model(state, sel_mask_pomo_aug)  # aug
                # shape: (batch, pomo)
