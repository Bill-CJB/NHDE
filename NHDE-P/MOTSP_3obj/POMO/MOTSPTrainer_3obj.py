...
class TSPTrainer:
    ...
    def _train_one_batch(self, batch_size):
        ...
        for st in range(self.trainer_params['solving_times']):
            hv_w = torch.rand(1)
            alpha = 1
            pref = numpy.random.dirichlet((alpha, alpha, alpha), None)
            ...
            s_[s_[:, :, 2] >= self.env_params['ref'][2]] = self.env_params['ref'][2].cuda() - 1e-4
            r_ = reward.clone()
            ...
            r_[r_[:, :, 2] >= self.env_params['ref'][2]] = self.env_params['ref'][2].cuda() - 1e-4
                    ...
                    hv_reward[b_i, i_i] = hypervolume(
                        torch.cat((s_[b_i, :, :], r_[b_i, i_i, None]), dim=0).cpu().numpy().astype(
                            float)).compute(self.env_params['ref'].numpy()) / (
                                                      self.env_params['ref'][0] * self.env_params['ref'][1] * self.env_params['ref'][2])
