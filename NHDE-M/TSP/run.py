...
# INTERNAL LIBRARY
####################################
from source.utilities import Get_Logger, Average_Meter, augment_xy_data_by_64_fold_2obj, augment_xy_data_by_32_fold_2obj
...
class Meta(nn.Module):
    ...
    def forward(self, epoch=0):
                        ...
                        for i_i in range(self.pomo_size):
                            hv_reward[b_i, i_i] = hypervolume(
                                torch.cat((s_[b_i, :, :], r_[b_i, i_i, None]), dim=0).cpu().numpy().astype(
                                    float)).compute(REF.numpy()) / (REF[0] * REF[1])
        ...
        for b_i in range(batch_s):
            hv[b_i] = hvwfg.wfg(sols[b_i][:NDS[b_i]].cpu().numpy().astype(float),
                                REF.numpy().astype(float)) / (REF[0] * REF[1])
    ...
    def finetune(self, pref=None, finetune_loader=None, model=None, hv_w=None, train_sols=None, train_sols_mask=None):
        ...
        for data in finetune_loader:
            ...
            assert AGG == 1 or AGG == 2, "Only support Weighted-Sum and Weighted-Tchebycheff"
            if AGG == 1:
                ws_reward = (pref * reward).sum(dim=2)
                agg_reward = ws_reward
            elif AGG == 2:
                z = torch.ones(reward.shape).cuda() * 0.0
                tch_reward = pref * (reward - z)
    ...
    def test(self, model=None, testdata=None, pref=None, test_sols=None, test_sols_mask=None):
        ...
        while True:
            ...
            with torch.no_grad():
                ...
                # test_model.reset(group_state, sel / 85 * 65, sel_mask, sel_mask_pomo)  # test MOTSP150
                # test_model.reset(group_state, sel / 115 * 65, sel_mask, sel_mask_pomo)  # test MOTSP200
    ...
    def test_aug(self, model=None, testdata=None, pref=None, test_sols_aug=None, test_sols_mask_aug=None):
        ...
        if aug_factor == 64:
            ...
            testdata_aug = augment_xy_data_by_32_fold_2obj(testdata)
        ...
        while True:
            ...
            testdata_batch = testdata_aug.reshape(aug_factor, -1, TSP_SIZE, 4)[:,
                             episode: episode + batch_s // aug_factor].reshape(batch_s, TSP_SIZE, 4)
            ...
            sel_mask = torch.cat((ref_mask, sel_mask), dim=-1)
            sel_mask_pomo = sel_mask[:, None, :].repeat(1, self.pomo_size, 1)
            sel_aug = sel[None, :, :, :].repeat(aug_factor, 1, 1, 1).reshape(batch_s, -1, 2)  # aug
            ...
            with torch.no_grad():
                ...
                # test_model_aug.reset(group_state, sel_aug / 85 * 65, sel_mask_aug, sel_mask_pomo_aug)  # test MOTSP150
                # test_model_aug.reset(group_state, sel_aug / 115 * 65, sel_mask_aug, sel_mask_pomo_aug)  # test MOTSP200
                ...
                # reward was negative, here we set it to positive
                reward = -reward
                reward = reward.reshape(aug_factor, -1, self.pomo_size, 2).permute(1, 0, 2, 3).reshape(
                    -1, aug_factor * self.pomo_size, 2)
                # update non-dominated solution set
                ...
                if sols.shape[1] > NEIGHBOR:
                    ...
                    split_flag = split_flag.scatter(1, ind, 1)
                    rest = sols[split_flag == 0].reshape(batch_s // aug_factor, -1, 2)
...
if MODE == 1:  # Train
    ...
    while True:
        ...
        test_sols_aug.append(torch.empty(batch_s, 0, 2))
    ...
    sols = torch.ones(TEST_DATASET_SIZE, 10000, 2) * 1000
    sols_aug = torch.ones(TEST_DATASET_SIZE, 10000, 2) * 1000
    episode = 0
    for t in test_sols:
...
elif MODE == 4:  # Load Submodels to Test
    ...
    testdata = testdata.to(device)
    test_save_ = 'test/tsp_size{}_FT{}'.format(TSP_SIZE, FINETUNE_STEP)
    ...
    for i in range(n_weight):
        ...
        test_timer_start = time.time()
        test_sols, test_sols_mask, test_NDS = meta_learner.test(model=actor, testdata=testdata, pref=pref[i], test_sols=test_sols, test_sols_mask=test_sols_mask)
        test_timer_end = time.time()
        total_test_time += test_timer_end - test_timer_start
        test_timer_start_aug = time.time()
        test_sols_aug, test_sols_mask_aug, test_NDS_aug = meta_learner.test_aug(model=actor, testdata=testdata, pref=pref[i], test_sols_aug=test_sols_aug, test_sols_mask_aug=test_sols_mask_aug)
        test_timer_end_aug = time.time()
        total_test_time_aug += test_timer_end_aug - test_timer_start_aug
