...
class Meta(nn.Module):
    ...
    def forward(self, epoch=0):
        ...
        for st in tqdm(range(SOLVING_TIMES)):
            ...
            for task_id in range(self.task_num):
                ...
                self.submodel.train()
                batch_i = 0
                for depot_xy, node_xy, node_demand in support_loader:
    ...
    def test_aug(self, model=None, testdata=None, pref=None, test_sols_aug=None, test_sols_mask_aug=None):
        ...
        if aug_factor == 8:
            ...
            aug_node_demand = node_demand.repeat(aug_factor, 1, 1)
        ...
        while True:
            ...
            depot_xy_batch = aug_depot_xy.reshape(aug_factor, -1, 1, 2)[:,
                             episode: episode + batch_s // aug_factor].reshape(batch_s, 1, 2)
            node_xy_batch = aug_node_xy.reshape(aug_factor, -1, PROBLEM_SIZE, 2)[:,
                            episode: episode + batch_s // aug_factor].reshape(batch_s, PROBLEM_SIZE, 2)
            node_demand_batch = aug_node_demand.reshape(aug_factor, -1, PROBLEM_SIZE, 1)[:,
                                episode: episode + batch_s // aug_factor].reshape(batch_s, PROBLEM_SIZE, 1)
            ...
            with torch.no_grad():
                ...
                group_s = PROBLEM_SIZE
                group_state, reward, done = env.reset(group_size=group_s)
                test_model_aug.reset(group_state, sel_aug, sel_mask_aug)
                ...
                while not done:
                    action_probs = test_model_aug.get_action_probabilities(group_state, sel_mask_pomo_aug)
                    # shape = (batch, group, problem+1)
                    action = action_probs.argmax(dim=2)
                    # shape = (batch, group)
                    action[group_state.finished] = 0  # stay at depot, if you are finished
...
if MODE == 1:  # Train
    ...
    for i in range(n_weight):
        ...
        test_sols_aug, test_sols_mask_aug, test_NDS_aug = meta_learner.test_aug(model=fine_model, testdata=testdata,
                                                                                pref=pref[i],
                                                                                test_sols_aug=test_sols_aug,
                                                                                test_sols_mask_aug=test_sols_mask_aug)
        test_timer_end_aug = time.time()
        total_test_time_aug += test_timer_end_aug - test_timer_start_aug
    sols = torch.ones(TEST_DATASET_SIZE, 1000, 2) * 1000
    sols_aug = torch.ones(TEST_DATASET_SIZE, 1000, 2) * 1000
    episode = 0
    for t in test_sols:
    ...
    for i in range(TEST_DATASET_SIZE):
        ...
        np.savetxt(os.path.join(test_save_dir, "sols_aug", "ins{}.txt".format(i)),
                   p_sols_aug[i, :p_sols_num_aug[i]].cpu().numpy(),
                   fmt='%1.4f\t%1.4f', delimiter='\t')
    print(MODEL_DIR)
